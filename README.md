...
- The root of the source tree should contain a file `./index.sst` which is really a markdown file which drives the generation of the home page (see below). 
...
The home page is generated using as input the file `./index.sst` in the root of the source tree. This is really a markdown file, whose title and subtitle field in the YAML header are used as blog title and subtitle, and which is converted to HTML using pandoc using template `<templates>/ssst-homepage.html`. The home page generated is stored in `./index.html` in the root of the destination tree.
