...
Even though the website is static, SSST also allows users to comment on posts through mail when they click on automatically generated `mailto` links embedded in posts and comments. These links ensure that the path to the post or comment replied to is automatically included in the subject field. This allows you to quickly insert the comment at the right place after moderation. And it makes even the comments/discussion part of the site completely self-hosted. This is an advantage compared to other self-hosted static website generators like [Hugo](https://gohugo.io).

SSST also processes LaTeX equations in a post, replacing them with SVG images containing the rendered equation in the output HTML. This uses `pdflatex` and `pdf2svg`. If an equation occurs on a single page more than once, only on image is generated and used for every occurrence. (This does not work across multiple pages containing the same equation.) Simple equations are handled 'inline' without requiring such an external image. In this case `pdflatex` and `pdf2svg` are not called. Sites with no or only simple mathematical equations therefore do not require these packages to be installed.

SSST keeps track of files changed during the process, and can create a batch file containing ftp (or other commands) to upload the changed files to the server. Useful in cases where you cannot keep the remote server files synchronised with the local files generated by SSST.
...
optional arguments:
  ...
  -u UPLOADSCRIPT, --uploadscript UPLOADSCRIPT
  ...
  -o, --overwrite       Overwrite files even when contents unchanged
```
...
The `ssst-comment` utility ads a comment received by email (as generated when a users submits a comment while clicking on SSST generated comment or reply link) in the appropriate file in the source tree. It uses the From and Date header in the email to set the author and date for the comment. It uses the Subject line to determine post and comment to which this comment needs to be attached. The contents of the email are used as comment text. If the message contains multiple mime parts, the text part is preferred. If that is missing, a html part is converted to plain text.
...
SSST processes markdown files containing posts or pages. SSST expects these files to start with a YAML block containing at least the post/page title (using the `title:` key). Posts have categories and tags. Pages do not. Posts are also expected to have a date (using the `date:` key). Pages do not (but can). Values for the date and title keys are strings (enclosed by single quotes).
...
The source tree (rooted at `SOURCE`) should be structured as follows:
...
- *Pages* are stored in folders with the following structure
  `./<optional path>/title/index.md`.  This is recommended but not enforced.
- *Posts* are typically stored in folders with the following structure
  `./yyyy/mm/dd/title/index.md`. This is recommended but not enforced.
...
Pages are essentially posts without categories or keywords. They do not show up (automatically) on the home page, and are not included in archives, category and tag pages. Therefore, to make them accessible, links to them need to be added explicitly, in posts, or through the templates.
...
are generated by SSST and should not be treated like original posts). These are stored in a separate tree rooted at `AUX`:
...
SSST generates the following output in the destination tree rooted at `DESTINATION`. All output is HTML generated using `pandoc` using different templates. 

- Posts are typically stored in
...
- Pages (essentially posts without tags and categories) are stored in
...
- Category pages are stored in
  ...
  and list links to all posts belonging to this category, in chronological order. The conversion uses template `<templates>/ssst-page.html`.
...
When generating posts or pages, any LaTeX equations (that are enclosed by \$..\$ in the markdown input) are automatically processed. Any equation that is 'too complex' to render natively (or almost any equation when specifying the `-g` or `--gladtex` option) will be converted using `pdflatex` and `pdf2svg`. The conversion process is controlled by a template `<templates>/ssst-eq.tex`, which should contain a complete LaTeX document, where the first occurance of `$$` is replace by the equation to convert. (The standard template uses the LaTeX standalone class.)
...
Unlike WordPress, any links to posts and pages generated by SSST contain an explicit `index.html` at the end where needed; this allows the generated content to be clicked through locally.


## Generating the home page

The home page is generated using as input the file `./index.md` in the root of the source tree. This markdown file, whose title and subtitle field in the YAML header are used as blog title and subtitle, is converted to HTML with pandoc using template `<templates>/ssst-homepage.html`. The home page generated is stored in `./index.html` in the root of the destination tree.
...
SSST keeps track of changed files, and does its best to remove any files it previously generated but are no longer necessary. To prevent unexpected deletion of a large section of files (when passing the wrong option for `DESTINATION`), files in `DESTINATION` are only deleted if it contains a file `ssst.canary` (that must be manually created).

Also, any folders in `DESTINATION` (eg folders containing style files, or content to offer for download, that are not generated using `ssst`) that contain a file named `KEEP` will not be deleted.

The `-u` option allows one to specify the name of an upload script, that SSST can add upload commands to (one for every file changed). The `-c` option allows one to specify the command to use to upload a changed file to the server. The command string can contain the following modifiers
...
- The Unix `head` command, to summarise posts, and `sed` to filter markdown input.
