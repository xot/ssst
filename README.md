...
SSST keeps track of changed files. The `-u` option allows one to specify the name of an upload script, that SSST can add upload commands to (one for every file changed). The `-c` option allows one to specify the command to use to upload a changed file to the server. The command string can contain the following modifiers
