...
- Media stored with post/page in the same directory are copied to the output tree (i.e files with extension `.jpg`, `.gif`, `.pdf` and `.png`)
  keeping their metadata (i.e modification times). In markdown these mediafiles should be referenced with a trailing ./ like "./name-of-image.png". Otherwise they will show up in post themselves but not elsewhere, for example the home page which needs to know where in de source tree the image is located.
